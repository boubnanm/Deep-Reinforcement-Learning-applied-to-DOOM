...
class DQNetwork(nn.Module):
    ...
    def __init__(self, w = 120, h = 160, init_zeros = False, stack_size = 4,out = 3):
        ...
        convw = conv2d_size_out(conv2d_size_out(conv2d_size_out(w))) # width of last conv output
        convh = conv2d_size_out(conv2d_size_out(conv2d_size_out(h))) # height of last conv output
...
class DDDQNetwork(nn.Module):
    ...
    def __init__(self, w = 120, h = 160, init_zeros = False, stack_size = 4, out = 3):
        ...
        # Value Module
        self.value_fc_1 = nn.Linear(linear_input_size, 1024)
        self.value_fc_2 = nn.Linear(1024, 512)
        self.value_output = nn.Linear(512, 1)
        
        # Advantage Module
        self.advantage_fc_1 = nn.Linear(linear_input_size, 1024)
